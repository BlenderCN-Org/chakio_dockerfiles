                ...
                for(int j=-1;j<2;j++)
                {
                    ...
                    else
                    {
                        ...
                        if(m[0]>0 && m[0]<=Potential.data.size() && m[1]>0 && m[1]<=Potential.data[0].size())
                        {
                            ...
                            //A*の重みの調整
                            double totalcost=TotalCost[n[0]][n[1]]-HCost[n[0]][n[1]]+HCost[m[0]][m[1]]+80*Cost+movementCost;
                            m[2]=0;
...
private:
    ros::NodeHandle              _nh;
    ros::ServiceServer service = _nh.advertiseService("pepper_pathGenerator",&setPotential::generatePath,this);
    ros::Publisher pubO        = _nh.advertise<sensor_msgs::PointCloud2> ("pepper_obstaclePotential", 1);
    ros::Subscriber sub_map    = _nh.subscribe("/pepper01/map",1,&setPotential::getMap,this);
    ros::Publisher goal_pub    = _nh.advertise<geometry_msgs::Pose2D> ("/goal_pose",2);
    potential map;//元のmapを格納用
    potential mapPotential;//mapの解像度をpotentialに合わせたmapPotential
    potential Potential;//統合されたポテンシャル
...
public:
    ...
    bool generatePath(printeps_pepper_modules::PathGenerator::Request  &req,
                      printeps_pepper_modules::PathGenerator::Response &res)
    {
        ...
        //既知障害物の考慮
        Potential=this->setStaticObstacle(Potential);
        //動的障害物の考慮
        ...
        //ゴールの決定
        cout<<"getGoal"<<endl;
        vector<double>goal=this->getGoal(Potential,req.start.x,req.start.y);
        //goal_pose = Pose2D();

        
        //goal_pose.x = goal[0];
        //goal_pose.y = goal[1];
        cout<< "リクエストゴール？" << req.goal.x << " " << req.goal.y;
        //ゴール時の姿勢計算
        double goal_theta=atan2(req.goal.y-goal[1],req.goal.x-goal[0]);
    ...
    potential setStaticObstacle(potential Potential)
    {
       ...
       //table1
        ...
        for(int i=0;i<2;i++)
        {
            chair[0]=Obstacles[2][0]+chairDistance*cos(3.14*i+Obstacles[2][3]);
            chair[1]=Obstacles[2][1]+chairDistance*sin(3.14*i+Obstacles[2][3]);
            chair[2]=chairRadius;
            chairObstacles.push_back(chair);
        }
        ...
        for(int i=0;i<2;i++)
        {
            chair[0]=Obstacles[3][0]+chairDistance*cos(3.14*i+Obstacles[3][3]);
            chair[1]=Obstacles[3][1]+chairDistance*sin(3.14*i+Obstacles[3][3]);
            chair[2]=chairRadius;
            chairObstacles.push_back(chair);
        }
        ...
        chair[0]=Obstacles[4][0]+chairDistance*cos(Obstacles[4][3]);
        chair[1]=Obstacles[4][1]+chairDistance*sin(Obstacles[4][3]);
        chair[2]=chairRadius;
        chairObstacles.push_back(chair);
        //table8
        table[0]=3.85;
        ...
        for (int i=0;i<obstaclePotential.data.size();i++)
        {
            for(int j=0;j<obstaclePotential.data[0].size();j++)
            {
                for(int k=0;k<obstaclePoses.size();k++)
                {
                    ...
                    {
                        obstaclePotential.data[i][j]=obstaclePotential.data[i][j]+this->func4(distance,obstaclePoses[k][2],Potential.resolution);
                    }
                }
            }
        }
        cout<<"setDynamicObstacle"<<endl;
        return obstaclePotential;
    }
        ...
        for (int i=0;i<GoalPotential.data.size();i++)
        {
            for(int j=0;j<GoalPotential.data[0].size();j++)
            {
                ...
                if(GoalPotential.data[i][j]<0.2)
                {
                    ...
                    {
                        GoalPotential.data[i][j]=GoalPotential.data[i][j]-this->func5(10,distance,Potential.resolution);
                    }
                }   
            }
        }
    ...
    vector<double> getGoal(potential Potential,double startX,double startY)
    {
        ...
        for (int i=0;i<Potential.data.size();i++)
        {
            for(int j=0;j<Potential.data[0].size();j++)
            {
                if(minimumValue*0.9>Potential.data[i][j])
                {
                    vector<int>point;
                    point.push_back(i);
                    point.push_back(j);
        ...
        cout << "どのゴール？"<< goal[0] << " " << goal[1] << endl;
        
  
        
        //ros::Publisher Goal_pub = nh.advertise<goal::Pose2D
    ...
    {
        double f=1/(1+exp(20*resolution*(x-r)));
        return f;
    }
...
{
  ros::init(argc, argv, "pathGeneratorService");
  ros::start(); 
