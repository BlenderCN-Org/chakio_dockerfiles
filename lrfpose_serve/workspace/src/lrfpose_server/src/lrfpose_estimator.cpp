...
#include "lrfpose_server/AMCLConfig.h"
...
class AmclNode
{
  ...
  //beam skip related params
    ...
    std::string camName;
    std::string camTfPrefix;
    int pairCamId;
    std::string camTfName;
        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("prefix", prifix_, std::string(""));
  private_nh_.param("laserHeight", laserHeight, 0.0);
  ROS_INFO("node name prefix:%s",prifix_.c_str());

  private_nh_.param("cam_name", camName, std::string(""));
  private_nh_.param("cam_tf_prefix", camTfPrefix, std::string(""));
  private_nh_.param("pair_cam", pairCamId, 0);
  std::stringstream ssCamTfName;
  ssCamTfName << camName << "0" << pairCamId << camTfPrefix;
  camTfName = ssCamTfName.str();
  ROS_INFO("camTfName:%s",camTfName.c_str());

  private_nh_.param("pos_noise", posNoise, 0.1);
  private_nh_.param("rot_noise", rotNoise, 0.1);
  ROS_INFO("pos_noise:%f",posNoise);
  ROS_INFO("rot_noise:%f",rotNoise);
  ...
  // publisher
  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>((prifix_+"/pose"), 2, true);
  particlecloud_pub_ = nh_.advertise<geometry_msgs::PoseArray>(prifix_+"/particlecloud", 2, true);
  ...
  // laser_scan_sub_ = nh_.subscribe<sensor_msgs::LaserScan>(nh_, scan_topic_, 100);
  // laser_scan_filter_ = 
  ...
  laser_check_interval_ = ros::Duration(1.0);
...
AmclNode::checkLaserReceived(const ros::TimerEvent& event)
{
  ...
  if(!calibrationStart)
  {
    ...
    try{
        tf_->lookupTransform( "map",  camTfName,ros::Time(0), tfFromMapToCamera);

        geometry_msgs::PoseWithCovarianceStamped cameraPose;
        cameraPose.pose.pose.position.x=tfFromMapToCamera.getOrigin().x();
        cameraPose.pose.pose.position.y=tfFromMapToCamera.getOrigin().y();
      
        tf::Matrix3x3 m(tfFromMapToCamera.getRotation());
        double roll, pitch, yaw;
        m.getRPY(roll, pitch, yaw);

        tf::Quaternion quat=tf::createQuaternionFromRPY(0,0,yaw+M_PI/2.0);
        ...
        ROS_INFO("Cam:%s, Setting pose (%.6f): %.3f %.3f %.3f",
        camTfName.c_str(),
        ros::Time::now().toSec(),
        tfFromMapToCamera.getOrigin().x(),
        tfFromMapToCamera.getOrigin().y(),
        yaw);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if( map_ == NULL  || calibrationStart == false) {
    return;
  }
  ...
  pf_vector_t pose;//pose:laserのオドメトリ
  ...
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    odom_->UpdateNoAction(pf_,posNoise,rotNoise);
      ...
      for(int i=0;i<set->sample_count;i++)
      {
        tf::poseTFToMsg(tf::Pose(tf::createQuaternionFromYaw(set->samples[i].pose.v[2]),
                                 tf::Vector3(set->samples[i].pose.v[0],
                                           set->samples[i].pose.v[1], laserHeight)),
                        cloud_msg.poses[i]);
      }
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      p.pose.pose.position.z = laserHeight;
      ...
      if (tf_broadcast_ == true)
      {
        ...
        tf::Vector3 tmpPos = tmp_tf_stamped.getOrigin();
        tmpPos.setZ(laserHeight);
        tmp_tf_stamped.setOrigin(tmpPos);
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ...
  for(int i=0; i<2; i++)
  {
    for(int j=0; j<2; j++)
    {
      pf_init_pose_cov.m[i][j] =5* msg.pose.covariance[6*i+j];
    }
  }
  pf_init_pose_cov.m[2][2] = 5*msg.pose.covariance[6*5+5];
