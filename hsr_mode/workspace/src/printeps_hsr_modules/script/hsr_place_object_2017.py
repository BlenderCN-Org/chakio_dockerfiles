...
TF_OBJECT   = ['ar_marker/10', 'ar_marker/10','ar_marker/9', 'ar_marker/12','ar_marker/13','ar_marker/8','ar_marker/9','ar_marker/14','hsr_wagon_tf','hsr_wagon_tf']   # つかむ物体のtf: ペットボトル、メニュー(棚)# 物体を置く場所のtf
...
#1
TARGET_POSE[WAGON][no1][PLACE][BEFORE]     = geometry.pose(x=0.05,y=-0.05-PetHeight-0.05, z=0.10,ei=-math.pi/2.0,ej=0, ek=-math.pi/2.0)   # #ｘ奥行き+が奥ｙ高さ＋が上ｚ横＋が左
TARGET_POSE[WAGON][no1][PLACE][CONTACT]    = geometry.pose(x=-0.05, z=0.10)
#TARGET_POSE[CART][no1][PICKUP][CONTACT]    = geometry.pose(x=0.12,y=-0.10, z=0.20, ek=-math.pi/2.0)     # テーブルの位置,z軸回に-90[deg]回転させた姿勢
TARGET_POSE[WAGON][no1][PLACE][AFTER]      = geometry.pose(x=0.05, z=-0.20)                    # 物体を置いた後ハンドを0.08[m]上に移動させ、0.2[m]後退した姿勢

#2
TARGET_POSE[WAGON][no2][PLACE][BEFORE]     = geometry.pose(x=0.05,y=-0.05-PetHeight-0.05, z=-0.10,ei=-math.pi/2.0,ej=0, ek=-math.pi/2.0)   # テーブルの手前0.12[m],z軸回に-90[deg]回転させた姿勢
TARGET_POSE[WAGON][no2][PLACE][CONTACT]    = geometry.pose(x=-0.05, z=0.10)
#TARGET_POSE[CART][no2][PICKUP][CONTACT]    = geometry.pose(x=0.27,y=-0.10, z=0.20, ek=-math.pi/2.0)     # テーブルの位置,z軸回に-90[deg]回転させた姿勢
TARGET_POSE[WAGON][no2][PLACE][AFTER]      = geometry.pose(x=0.05, z=-0.20)                    # 物体を置いた後ハンドを0.08[m]上に移動させ、0.2[m]後退した姿勢

#3
TARGET_POSE[WAGON][no3][PLACE][BEFORE]     = geometry.pose(x=-0.05,y=-0.05-PetHeight-0.02, z=0.10,ei=-math.pi/2.0,ej=0, ek=-math.pi/2.0)   # テーブルの手前0.12[m],z軸回に-90[deg]回転させた姿勢
TARGET_POSE[WAGON][no3][PLACE][CONTACT]    = geometry.pose(x=-0.05, z=0.10)     # テーブルの位置,z軸回に-90[deg]回転させた姿勢
#TARGET_POSE[CART][no3][PICKUP][CONTACT]    = geometry.pose(x=0.12,y=-0.10, z=0.06, ek=-math.pi/2.0) 
TARGET_POSE[WAGON][no3][PLACE][AFTER]      = geometry.pose(x=0.05, z=-0.20)                    # 物体を置いた後ハンドを0.08[m]上に移動させ、0.2[m]後退した姿勢

#4
TARGET_POSE[WAGON][no4][PLACE][BEFORE]     = geometry.pose(x=-0.05,y=-0.05-PetHeight-0.02, z=-0.10,ei=-math.pi/2.0,ej=0, ek=-math.pi/2.0)   # テーブルの手前0.12[m],z軸回に-90[deg]回転させた姿勢
TARGET_POSE[WAGON][no4][PLACE][CONTACT]    = geometry.pose(x=-0.05, z=0.10)     # テーブルの位置,z軸回に-90[deg]回転させた姿勢
#TARGET_POSE[CART][no4][PICKUP][CONTACT]    = geometry.pose(x=0.27,y=-0.10, z=0.06, ek=-math.pi/2.0)
TARGET_POSE[WAGON][no4][PLACE][AFTER]      = geometry.pose(x=0.05, z=-0.20)                    # 物体を置いた後ハンドを0.08[m]上に移動させ、0.2[m]後退した姿勢


# 置くのためのパラメータ(PLACE　HANDOVER) ---------------------------------------------------------
TARGET_POSE[HANDOVER][HANDOVER][PLACE][BEFORE]     = geometry.pose(x=0,y=-0.05-PetHeight-0.05-0.03, z=0.04, ek=-math.pi/2.0)   # テーブルの手前0.12[m],z軸回に-90[deg]回転させた姿勢
# 置くのためのパラメータ(PLACE　TABLE1) ---------------------------------------------------------

#1
TARGET_POSE[TABLE1][no1][PLACE][BEFORE]     = geometry.pose(x=-0.24,y=-0.05-PetHeight-0.08, z=0.08, ek=-math.pi/2.0)   # テーブルの手前0.12[m],z軸回に-90[deg]回転させた姿勢
TARGET_POSE[TABLE1][no1][PLACE][CONTACT]    = geometry.pose(x=-0.05, z=0.15)     # テーブルの位置,z軸回に-90[deg]回転させた姿勢
...
#2
TARGET_POSE[TABLE1][no2][PLACE][BEFORE]     = geometry.pose(x=-0.08,y=-0.05-PetHeight-0.08, z=0.08, ek=-math.pi/2.0)   # テーブルの手前0.12[m],z軸回に-90[deg]回転させた姿勢
TARGET_POSE[TABLE1][no2][PLACE][CONTACT]    = geometry.pose(x=-0.05, z=0.15)     # テーブルの位置,z軸回に-90[deg]回転させた姿勢
...
#3
TARGET_POSE[TABLE1][no3][PLACE][BEFORE]     = geometry.pose(x=0.08,y=-0.05-PetHeight-0.08, z=0.08, ek=-math.pi/2.0)   # テーブルの手前0.12[m],z軸回に-90[deg]回転させた姿勢
TARGET_POSE[TABLE1][no3][PLACE][CONTACT]    = geometry.pose(x=-0.05, z=0.15)     # テーブルの位置,z軸回に-90[deg]回転させた姿勢
...
#4
TARGET_POSE[TABLE1][no4][PLACE][BEFORE]     = geometry.pose(x=0.24,y=-0.05-PetHeight-0.08, z=0.08, ek=-math.pi/2.0)   # テーブルの手前0.12[m],z軸回に-90[deg]回転させた姿勢
TARGET_POSE[TABLE1][no4][PLACE][CONTACT]    = geometry.pose(x=-0.05, z=0.15)     # テーブルの位置,z軸回に-90[deg]回転させた姿勢
...
class HSRPlaceObject:
    ...
    def __init__(self):
        ...
        self.Handover=False # trueのとき手渡しする
        # Initialize node ------------------------------------------------------
    ...
    def placeObjectHandler(self, object):
        ...
        elif object.place_id == 'Wagon1':
            whole_body.linear_weight=(50)
            self.place_id  = WAGON
            self.no =no1
            print TF_OBJECT[self.place_id]
            print "Target Place: WAGON1"
        elif object.place_id == 'Wagon2':
            whole_body.linear_weight=(50)
            self.place_id  = WAGON
            self.no =no2
            print "Target Place: WAGON2"
        elif object.place_id == 'Wagon3':
            whole_body.linear_weight=(5)
            self.place_id  = WAGON
            self.no =no3
            print "Target Place: WAGON3"
        elif object.place_id == 'Wagon4':
            whole_body.linear_weight=(5)
            self.place_id  = WAGON
            self.no =no4
            print "Target Place: WAGON4"
        ...
        #Trash
        elif object.place_id == 'Trash':
            whole_body.linear_weight=(5)
            self.place_id  = TRASH
            self.no =TRASH
            print "Target Place: TRASH"
    ...
    def performPlaceObject(self):
        ...
        if self.place_id == TRASH:
            talkingWords="ゴミを捨てます"
        elif self.place_id != CART and self.place_id != WAGON:
            publog.publish("start,place")
            talkingWords= self.object_name + "をお持ちしました"
        else:
            talkingWords=""
        ...
        if self.place_id==WAGON:
            ...
            try:    
                if self.Handover==True:
                    ...
                    if self.place_id==CART1 or self.place_id==CART2 or self.place_id==CART3 or self.place_id==CART4:
