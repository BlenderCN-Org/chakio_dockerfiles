...
if __name__ == '__main__':
    log_cmd_publisher      = rospy.Publisher('hsr_logging_command', String, queue_size=50)
    ...
    # ServiceClient ------------------------------------------------------------
        # PickAndPlace Server --------------------------------------------------
    pickandplace_client = rospy.ServiceProxy('_hsr_pick_and_place', HsrPickAndPlace)
        # Carry Server ---------------------------------------------------------
    carry_client = rospy.ServiceProxy('_hsr_carry', HsrCarry2017)
    ...
    # phase:4 カートを棚の前に移動させる
        ...
        msg = String()
        msg.data = 'start'
        log_cmd_publisher.publish("start")
        # 2つのペットボトルをtable1に運ぶ
        # phase:1 ペットボトルをカートに置く
        # 1 ボトル1からカート1へ運ぶ
        HsrPickAndPlace.object_place = 'Pet1'
        HsrPickAndPlace.destination  = 'Wagon3'
        HsrPickAndPlace.object_name  = 'お茶'   
        pickandplace_client(HsrPickAndPlace.object_place,HsrPickAndPlace.destination,HsrPickAndPlace.object_name)

        #2　ボトル2からカート2へ運ぶ
        HsrPickAndPlace.object_place = 'Pet2'
        HsrPickAndPlace.destination  = 'Wagon4'
        HsrPickAndPlace.object_name  = 'お茶'   
        pickandplace_client(HsrPickAndPlace.object_place,HsrPickAndPlace.destination,HsrPickAndPlace.object_name)
        
        # phase:2 カートをテーブルの前に移動させる
        HsrCarry2017.pose_B = 'Pet'                             # 棚の絶対座標
        HsrCarry2017.pose_D = 'Table1'                          # table4の絶対座標
        carry_client(HsrCarry2017.pose_B,HsrCarry2017.pose_D)

        # phase:3 カートからテーブルにペットボトルを移動させる
        #1 カート1からtable41に運ぶ
        HsrPickAndPlace.object_place = 'Wagon3'
        HsrPickAndPlace.destination  = 'Table11'
        HsrPickAndPlace.object_name  = 'お茶'     
        pickandplace_client(HsrPickAndPlace.object_place,HsrPickAndPlace.destination,HsrPickAndPlace.object_name)

        #2 カート1からtable42に運ぶ
        HsrPickAndPlace.object_place = 'Wagon4'
        HsrPickAndPlace.destination  = 'Table14'
        HsrPickAndPlace.object_name  = 'お茶'   
        pickandplace_client(HsrPickAndPlace.object_place,HsrPickAndPlace.destination,HsrPickAndPlace.object_name)
        
        log_cmd_publisher.publish("end")