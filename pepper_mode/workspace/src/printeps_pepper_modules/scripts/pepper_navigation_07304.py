...
TARGET_POSITION_TOLERANCE   = 0.15 #0.3                      # 到着判定距離[m]
...
FUZZY_RNG_NUM               = 180
AVOID_T_DIST                = 1.25
NEAR_GOAL_DIST              = 0.5 #1.0                   # 減速開始距離[m]
NEAR_NEAR_GOAL_DIST         = NEAR_GOAL_DIST #NEAR_GOAL_DIST/1.5    # 障害物回避を無効にする距離[m]
...
class PepperNavigation:
    ...
    def __init__(self):
        ...
        if self.use_virtual_object==True:
            ...
            '''
            # カウンター
            self.r_object[0].p_x    = 0.0
            self.r_object[0].p_y    = -1.8
            self.r_object[0].len_x  = 1.2
            self.r_object[0].len_y  = 1.0

            # テーブル1
            self.r_object[1].p_x    = 2.8
            self.r_object[1].p_y    = -1.8
            self.r_object[1].len_x  = 2.0
            self.r_object[1].len_y  = 1.2

            # テーブル2
            self.r_object[2].p_x    = 4.0
            self.r_object[2].p_y    = -1.8
            self.r_object[2].len_x  = 2.0
            self.r_object[2].len_y  = 1.2

            # テーブル3
            self.r_object[3].p_x    = 2.8
            self.r_object[3].p_y    = 0.8
            self.r_object[3].len_x  = 1.0
            self.r_object[3].len_y  = 1.5

            # テーブル4
            self.r_object[4].p_x    = 4.0
            self.r_object[4].p_y    = 1.5
            self.r_object[4].len_x  = 1.2
            self.r_object[4].len_y  = 1.2

            
            # カウンターJACO
            self.r_object[5].p_x    = 0.0
            self.r_object[5].p_y    = 12.0
            self.r_object[5].len_x  = 2.0
            self.r_object[5].len_y  = 1.0
            

            # テーブル3にスタックしないための障害物
            self.r_object[6].p_x    = 1.0
            self.r_object[6].p_y    = 12.0#1.7
            self.r_object[6].len_x  = 2.0
            self.r_object[6].len_y  = 1.0
    ...
    def makeTMixMF(self, in_dist):
        ...
        for i in range(FUZZY_RNG_NUM):
            if in_dist > NEAR_NEAR_GOAL_DIST:
                ...
                self.mbsp_t_mix[i]  = min(self.mbsp_t_goal[i], self.mbsp_t_obst[i])
            else:
    ...
    def navigationHandler(self, goal):
        ...
        while (reached_t_flg == False or reached_r_flg == False):
            ...
            if motion_phase == ROT1:
                ...
                if the_dist < TARGET_POSITION_TOLERANCE:
                    motion_phase    = ROT2
                    reached_t_flg   = True
            ...
            # フェーズ2: 移動 *****************************************************
                ...
                #print cmd_vel
                print "vx=", cmd_vel.linear.x, "vy=", cmd_vel.linear.y
