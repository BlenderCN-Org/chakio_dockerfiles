...
private:
    ...
    ros::Timer timer                    = _nh.createTimer(ros::Duration(0.1), &lrfPoseServer::timerCallback,this);
...
public:
    lrfPoseServer()
    {
        ...
        while(std::getline(readingCSV,oneLine))
        {
            ...
            if(valueIndex>0)
            {
                //tf::Vector3 axis(quaternionValue[0],quaternionValue[1],quaternionValue[2]);
                //tf::Quaternion quaternion(axis,quaternionValue[3]);
                tf::Quaternion quaternion(rpyValue[0],rpyValue[1],rpyValue[2]);
                tf::Transform lrfPose;
                lrfPose.setOrigin(position);
                lrfPose.setRotation(quaternion.normalize());
                lrfPoses.push_back(lrfPose);
                std::cout<<"quaternion    x:"<<quaternionValue[0]<<" y:"<<quaternionValue[1]<<" z:"<<quaternionValue[2]<<" w:"<<quaternionValue[3]<<std::endl;
                std::cout<<"quaternion    x:"<<quaternion.getAxis().x()<<" y:"<<quaternion.getAxis().y()<<" z:"<< quaternion.getAxis().z()<<" w:"<< quaternion.getW()<<std::endl;
                std::cout<<"lrfPoses    x:"<<lrfPose.getRotation().getAxis().x()<<" y:"<<lrfPose.getRotation().getAxis().y()<<" z:"<< lrfPose.getRotation().getAxis().z()<<" w:"<< lrfPose.getRotation().getW()<<std::endl;
                lineIndex++;
            }
        }
        for(int tfIndex=0;tfIndex<lrfPoses.size();tfIndex++)
        {
            ...
            std::cout<<"        x:"<<lrfPoses[tfIndex].getOrigin().getX()<<" y:"<<lrfPoses[tfIndex].getOrigin().getY()<<" z:"<<lrfPoses[tfIndex].getOrigin().getZ()<<std::endl;
            std::cout<<"        x:"<<lrfPoses[tfIndex].getRotation().getAxis().x()<<" y:"<<lrfPoses[tfIndex].getRotation().getAxis().y()<<" z:"<< lrfPoses[tfIndex].getRotation().getAxis().z()<<" w:"<< lrfPoses[tfIndex].getRotation().getW()<<std::endl;
            double roll,pitch,yaw;
            tf::Matrix3x3(lrfPoses[tfIndex].getRotation()).getRPY(roll, pitch, yaw);
    ...
    {
        for(int lrfPoseIndex=0;lrfPoseIndex<lrfPoses.size();lrfPoseIndex++)
        {
    ...
    void broadcastLrfPose(tf::Transform lrfPose,int lrfPoseIndex)
    {
        ...
        _br.sendTransform(tf::StampedTransform(lrfPose, ros::Time::now(),"map",lrfLinkName.str()));
    }
};
