...
################################################################################
# 移動のためのパラメータ ---------------------------------------------------------
MOTION_TIME_TORELANCE       = 90.0                     # 移動タイムアウト時間[s]
TARGET_POSITION_TOLERANCE   = 0.3                      # 到着判定距離[m]
TARGET_ANGULAR_TOLERANCE    = (20.0/180.0*math.pi)      # 到着判定角度[rad]
...
FUZZY_RNG_NUM               = 180
AVOID_T_DIST                = 1.25
NEAR_GOAL_DIST              = 1.0                   # 減速開始距離[m]
NEAR_NEAR_GOAL_DIST         = NEAR_GOAL_DIST/1.5    # 障害物回避を無効にする距離[m]
REACHED_GOAL_DIST           = 0.3                   # 目標し正確に修正し始める距離[m]
OBST_MARGIN                 = 0.2
R_ROBOT                     = 0.25

MAX_LINEAR_VELOCITY         = 0.4   # 並進最大速度[m/s]　デフォルト
MAX_ANGULAR_VELOCITY        = 0.8   # 回転最大速度[rad/s]　デフォルト
...
class PepperNavigation:
    ...
    def __init__(self):
        ...
        # Initialize node ------------------------------------------------------
        rospy.init_node("printeps_pepper_modules")
        ...
        # 仮想障害物の設定 -------------------------------------------------------
        self.use_virtual_object = rospy.get_param('~use_virtual_object', False)
        ...
        if self.use_virtual_object==True:
            ...
            '''
            # カウンター
            self.r_object[0].p_x    = 1.6+0.2
            self.r_object[0].p_y    = -0.7 - 0.4
            self.r_object[0].len_x  = 3.2
            self.r_object[0].len_y  = 0.8

            # テーブル1
            self.r_object[1].p_x    = 5.0
            self.r_object[1].p_y    = -0.7 -0.8
            self.r_object[1].len_x  = 3.0   #0.8
            self.r_object[1].len_y  = 1.6

            # テーブル2
            self.r_object[2].p_x    = 7.2
            self.r_object[2].p_y    = -0.7 -0.8
            self.r_object[2].len_x  = 3.0   #0.8
            self.r_object[2].len_y  = 1.6

            # テーブル3
            self.r_object[3].p_x    = 5.0
            self.r_object[3].p_y    = 2.0
            self.r_object[3].len_x  = 0.6 -0.1
            self.r_object[3].len_y  = 1.4

            # テーブル4
            self.r_object[4].p_x    = 7.2
            self.r_object[4].p_y    = 2.0
            self.r_object[4].len_x  = 0.6 - 0.1
            self.r_object[4].len_y  = 1.4

            
            # テーブル3にスタックしないための障害物
            self.r_object[5].p_x    = 2.5
            self.r_object[5].p_y    = 2.0
            self.r_object[5].len_x  = 3.0
            self.r_object[5].len_y  = 1.2
    ...
    def launchServer(self):
        ...
        # LRS Data Subscriber --------------------------------------------------
        lrs_subscriber  = rospy.Subscriber('lrs_front/scan', LaserScan, self.lrsCallback)

        # Current pose Subscriber ----------------------------------------------
        amcl_subscriber = rospy.Subscriber('amcl_pose', PoseWithCovarianceStamped, self.amclCallback)

        # Navigation Server ----------------------------------------------------
        nav_server  = rospy.Service('pepper_navigation', PepperNav, self.navigationHandler)
    ...
    def navigationHandler(self, goal):
        ...
        if the_dist > NEAR_GOAL_DIST:
            motion_phase    = ROT1              # 回転方向を向くフェーズ
        else:
            motion_phase    = MOVE              # そのまま移動
        ...
        while (reached_t_flg == False or reached_r_flg == False):
            ...
            if motion_phase == ROT1:
                ...
                print "[ROT1]目標角度まで:", the_dir_angle*180.0/math.pi, "[deg]"
            ...
            # フェーズ3: 最終姿勢角 ************************************************
                ...
                print "[ROT2]目標角度まで:", (self.current_pose.theta - goal.pose.theta)*180.0/math.pi, "[deg]"
        ...
        if reached_t_flg == True and reached_r_flg == True:
            ...
            return PepperNavResponse(success=True)
        else:
            print "False: Did Not Reach Goal\n"
            return PepperNavResponse(success=False)
